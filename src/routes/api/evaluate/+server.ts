...
import type { Classification, EvaluateConflictResponse, EvaluateResponse } from '$lib/types';
...
export const POST: RequestHandler = async ({ request, getClientAddress, cookies }) => {
  ...
  const aiScore = await loadSeedScore(session.claimId, input.cardId);
  ...
  // re-rule a card. ai_reaction_text starts NULL; /api/reaction fills it
  // in async so the client can advance to the next card without waiting on
  // the LLM call.
  const { data: pickInsert, error: pickError } = await suspicion
    .from('picks')
    .insert({
  ...
  if (pickError || !pickInsert) {
    ...
    if ((pickError as { code?: string } | null)?.code === '23505') {
    ...
    console.error('[evaluate] picks insert failed:', pickError?.message ?? 'no row returned');
  ...
  return json({
    pick_id: pickInsert.id as string,
